...
import { absVal, add, addN, div, ifCond, lt, max, maxN, min, minN, mul, neg, sqrt, squared, sub, } from "../engine/AutodiffFunctions.js";
import * as BBox from "../engine/BBox.js";
import { noWarn, noWarnFn, real2NT, real2T, realNT, realT, shapeT, } from "../utils/Util.js";
import { constrDictCurves } from "./Curves.js";
import { absCircleToImplicitEllipse, absEllipseToImplicit, } from "./ImplicitShapes.js";
import { containsConvexPolygonPoints, convexPartitions, overlappingImplicitEllipses, } from "./Minkowski.js";
import { bboxFromShape, bboxPts, polygonLikePoints, shapeDistance, } from "./Queries.js";
import * as utils from "./Utils.js";
import { isRectlike, overlap1D, relu, toPt } from "./Utils.js";
export const andConstraint = (...xs) => {
 ...
 */
export const disjointScalar = (c, left, right) => {
    const d = (x, y) => absVal(sub(x, y));
 ...
 */
export const perpendicular = (q, p, r) => {
 ...
 */
export const collinear = (c1, c2, c3) => {
 ...
 */
export const collinearOrdered = (c1, c2, c3) => {
...
export const onCanvas = (shape, canvasWidth, canvasHeight) => {
    ...
    return add(contains1D(canvasXRange, BBox.xRange(box)), contains1D(canvasYRange, BBox.yRange(box)));
};
export const overlapping = (s1, s2, overlap = 0) => {
    ...
    if (t1 === "Ellipse" && t2 === "Ellipse")
        return noWarn(overlappingEllipses(toPt(s1.center.contents), s1.rx.contents, s1.ry.contents, toPt(s2.center.contents), s2.rx.contents, s2.ry.contents, overlap));
    // Circle x Ellipse
    else if (t1 === "Circle" && t2 === "Ellipse")
        return noWarn(overlappingCircleEllipse(toPt(s1.center.contents), s1.r.contents, toPt(s2.center.contents), s2.rx.contents, s2.ry.contents, overlap));
    else if (t1 === "Ellipse" && t2 === "Circle")
        return noWarn(overlappingCircleEllipse(toPt(s2.center.contents), s2.r.contents, toPt(s1.center.contents), s1.rx.contents, s1.ry.contents, overlap));
...
export const overlappingEllipses = (c1, rx1, ry1, c2, rx2, ry2, overlap) => {
    const d = ops.vdist(c1, c2);
    const factor = div(1, add(1, d));
    const ei1 = absEllipseToImplicit(c1, rx1, ry1, neg(overlap), mul(Math.PI / 3, factor));
    const ei2 = absEllipseToImplicit(c2, rx2, ry2, 0, factor);
    return overlappingImplicitEllipses(ei1, ei2);
};
export const overlappingCircleEllipse = (c1, r1, c2, rx2, ry2, overlap) => {
    const d = ops.vdist(c1, c2);
    const factor = div(1, add(1, d));
    const ei1 = absCircleToImplicitEllipse(c1, r1, neg(overlap), mul(Math.PI / 3, factor));
    const ei2 = absEllipseToImplicit(c2, rx2, ry2, 0, factor);
    return overlappingImplicitEllipses(ei1, ei2);
};
export const disjoint = (s1, s2, padding = 0) => {
...
export const touching = (s1, s2, padding = 0) => {
    const { value: overlap, warnings } = overlapping(s1, s2, neg(padding));
    return {
...
export const contains = (s1, s2, padding = 0.0) => {
    const t1 = s1.shapeType, t2 = s2.shapeType;
    if (t1 === "Circle" && t2 === "Circle")
        return noWarn(containsCircles(toPt(s1.center.contents), s1.r.contents, toPt(s2.center.contents), s2.r.contents, padding));
    else if (t1 === "Polygon" && t2 === "Polygon") {
        return noWarn(containsPolys(polygonLikePoints(s1), polygonLikePoints(s2), padding));
    }
    else if (t1 === "Polygon" && t2 === "Circle") {
        return noWarn(containsPolyCircle(polygonLikePoints(s1), toPt(s2.center.contents), s2.r.contents, padding));
    }
    else if (t1 === "Circle" && t2 === "Polygon") {
        return noWarn(containsCirclePoly(toPt(s1.center.contents), s1.r.contents, polygonLikePoints(s2), padding));
    }
    else if (t1 === "Circle" && isRectlike(s2)) {
        const rPts = bboxPts(bboxFromShape(s2));
        return noWarn(containsCircleRect(toPt(s1.center.contents), s1.r.contents, rPts, padding));
    }
    else if (isRectlike(s1) && t2 === "Circle") {
        const rPts = bboxPts(bboxFromShape(s1));
        return noWarn(containsRectCircle(rPts, toPt(s2.center.contents), s2.r.contents, padding));
    }
    else if (t1 === "Group") {
        const { value, warnings } = containsGroupShape(s1.shapes.contents, s1.clipPath.contents, s2, padding);
        return {
            value,
            warnings: warnings.map((warning) => {
                if (warning.tag === "BBoxApproximationWarning") {
        ...
        return noWarn(containsRects(bboxPts(bboxFromShape(s1)), bboxPts(bboxFromShape(s2)), padding));
    }
    else {
...
export const containsRectCircle = (rect, c, r, padding) => {
    if (rect.length !== 4) {
        throw new Error("`rect` should be a list of four 2d-points.");
    }
    const [, tl, , br] = rect;
    ...
    //    max( |cx-rx| - (w/2-r-o),
    //         |cy-ry| - (h/2-r-o) )
    return max(sub(absVal(sub(cx, rx)), sub(sub(halfW, r), padding)), sub(absVal(sub(cy, ry)), sub(sub(halfH, r), padding)));
};
export const containsRects = (rect1, rect2, padding) => {
    // TODO: add padding.
...
export const containsGroupShape = (shapes, clip, s2, padding) => {
            ...
            if (warning.tag === "BBoxApproximationWarning") {
                const newStack = [
                    ...warning.stack,
                    {
                        signature: `containsGroupShape([${shapes
                            .map((s) => s.shapeType)
                            .join(", ")}], clip(${clip.contents.shapeType}), ${s2.shapeType}, ...)`,
                    },
                ];
                return {
                    ...warning,
 ...
 */
export const disjointIntervals = (s1, s2) => {
    return overlap1D([s1.start.contents[0], s1.end.contents[0]], [s2.start.contents[0], s2.end.contents[0]]);
};
//#endregion
// -------- Simple constraints
...
const constrDictSimple = {
    ...
    lessThan: {
        name: "lessThan",
        description: "Require that the value `x` is less than the value `y` with optional padding `padding`",
        params: [
            { name: "x", description: "First value", type: realT() },
            { name: "y", description: "Second value", type: realT() },
    ...
    greaterThan: {
        name: "greaterThan",
        description: "Require that the value `x` is greater than the value `y` with optional padding `padding`",
        params: [
            { name: "x", description: "First value", type: realT() },
            { name: "y", description: "Second value", type: realT() },
            { name: "padding", description: "Padding", type: realT(), default: 0 },
    ...
    lessThanSq: {
        name: "lessThanSq",
        description: "Require that the value `x` is less than the value `y`, with steeper penalty",
        params: [
            { name: "x", description: "First value", type: realT() },
            { name: "y", description: "Second value", type: realT() },
        ],
    ...
    greaterThanSq: {
        name: "greaterThanSq",
        description: "Require that the value `x` is greater than the value `y`, with steeper penalty",
        params: [
            { name: "x", description: "First value", type: realT() },
            { name: "y", description: "Second value", type: realT() },
        ],
    ...
    inRange: {
        name: "inRange",
        description: "Require that the value `x` is in the range defined by `[x0, x1]`.",
        params: [
    ...
    contains1D: {
        name: "contains1D",
        description: "Require that an interval `[l1, r1]` contains another interval `[l2, r2]`. If not possible, returns 0.",
        params: [
            {
                name: "[l1, r1]",
    ...
    perpendicular: {
        name: "perpendicular",
        description: "Require that the vector defined by `(q, p)` is perpendicular from the vector defined by `(r, p)`.",
        params: [
   ...
   (e.g. if \`s1\` should be overlapping \`s2\` with margin \`overlap\`).`,
    ...
    containsCircleRect: {
        ...
        params: [
            ...
            {
                name: "rect",
                description: "The top-right, top-left, bottom-left, bottom-right points (in that order) of rectangle `rect`",
                type: real2NT(),
            },
            {
                name: "padding",
    ...
    containsRects: {
        ...
        params: [
            {
                name: "rect1",
                description: "The top-right, top-left, bottom-left, bottom-right points (in that order) of rectangle `rect1`",
                type: real2NT(),
            },
            {
                name: "rect2",
                description: "The top-right, top-left, bottom-left, bottom-right points (in that order) points of rectangle `rect2`",
                type: real2NT(),
            },
            {
                name: "padding",
...
const constrDictSpecific = {
    disjointIntervals: {
        ...
        description: "Make two intervals disjoint. They must be 1D intervals (line-like shapes) sharing a y-coordinate.",
        params: [
...
const _constrDictVals = Object.values(constrDict);
