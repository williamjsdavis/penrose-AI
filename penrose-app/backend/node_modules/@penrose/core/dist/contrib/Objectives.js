...
import { absVal, add, addN, div, ifCond, inverse, lt, max, mul, neg, squared, sub, } from "../engine/AutodiffFunctions.js";
import { booleanT, linePts, noWarnFn, real2T, realNMT, realNT, realT, rectlikeT, shapeT, unionT, } from "../utils/Util.js";
...
import { closestPt_PtSeg, isLinelike, repelPoint, sampleSeg, } from "./Utils.js";
...
export const objDictSimple = {
    ...
    equal: {
        ...
        body: noWarnFn((x, y) => squared(sub(x, y))),
    },
    nearVec: {
        name: "nearVec",
        description: "Encourage two vectors `v1` and `v2` to be near each other with distance `offset`.",
        params: [
    ...
    greaterThan: {
        name: "greaterThan",
        description: "Encourage x to be greater than or equal to y: `max(0,y - x)^2`",
        params: [
            { name: "x", description: "First value", type: realT() },
            { name: "y", description: "Second value", type: realT() },
        ],
        body: noWarnFn((x, y) => squared(max(0, sub(y, x)))),
    },
    /**
    ...
    lessThan: {
        ...
        body: noWarnFn((x, y) => squared(max(0, sub(x, y)))),
    },
    /**
    ...
    repelPt: {
        name: "repelPt",
        description: "Repel point `a` from another scalar `b` with weight `weight`.",
        params: [
        ...
        body: noWarnFn((weight, a, b) => mul(weight, inverse(add(ops.vdistsq(a, b), EPS_DENOM)))),
    },
    /**
...
export const objDictGeneral = {
    inDirection: {
        name: "inDirection",
        description: "Encourage the point `p` to be in the direction `direction` with respect to point `pRef`. The `direction` vector does not need to be normalized. The `offset` parameter is the shortest allowed distance between the points.",
        params: [
            { name: "p", type: real2T() },
    ...
    below: {
        name: "below",
        description: "Encourage the center of `bottom` to be below the center of `top`.",
        params: [
            {
                name: "bottom",
        ...
        body: noWarnFn((bottom, top, offset = 100) => {
    ...
    above: {
        name: "above",
        description: "Encourage the center of `top` to be above the center of `bottom`.",
        params: [
            {
                name: "top",
                type: shapeT("AnyShape"),
        ...
        body: noWarnFn((top, bottom, offset = 100) => {
    ...
    leftwards: {
        name: "leftwards",
        description: "Encourage the center of `left` to be leftwards to the center of `right`.",
        params: [
            {
                name: "left",
                type: shapeT("AnyShape"),
        ...
        body: noWarnFn((left, right, offset = 100) => {
    ...
    rightwards: {
        name: "rightwards",
        description: "Encourage the center of `right` to be rightwards to the center of `left`.",
        params: [
            {
                name: "right",
                type: shapeT("AnyShape"),
        ...
        body: noWarnFn((right, left, offset = 100) => {
    ...
    sameCenter: {
        name: "sameCenter",
        description: "Encourage shape `s1` to have the same center position as shape `s2`.",
        params: [
    ...
    notTooClose: {
        ...
        body: noWarnFn((s1, s2, weight = 10.0) => {
            ...
            if (isLinelike(s1)) {
                ...
                const allForces = addN(lineSamplePts.map((p) => repelPoint(weight, c2, p)));
                res = mul(weight, allForces);
            }
            else {
                // Repel any two shapes with a center.
                // 1 / (d^2(cx, cy) + eps)
                res = inverse(add(ops.vdistsq(shapeCenter(s1), shapeCenter(s2)), EPS_DENOM));
            }
    ...
    near: {
        name: "near",
        description: "Try to place shape `s1` near shape `s2` (putting their centers at the same place).",
        params: [
            { name: "s1", type: shapeT("AnyShape"), description: "a shape" },
            { name: "s2", type: shapeT("AnyShape"), description: "a shape" },
            {
        ...
        body: noWarnFn((s1, s2, offset = 10.0) => {
    ...
    nonDegenerateAngle: {
        ...
        body: noWarnFn((s0, s1, s2, strength = 20, range = 10) => {
            ...
            return ifCond(lt(cosine, range * (Math.PI / 180)), 0, mul(strength, cosine));
        }),
    },
};
...
export const objDictSpecific = {
    centerLabelAbove: {
        ...
        body: noWarnFn((s1, s2, w) => {
            ...
            const rh = squared(sub(add(my, mul(textBB.height, 1.1)), textBB.center[1]));
    ...
    centerLabel: {
        ...
        body: noWarnFn((s1, s2, w, padding = 10) => {
            if (isLinelike(s1)) {
                ...
                return add(sub(ops.vdistsq(midpt, textBB.center), squared(textBB.width)), squared(padding));
            }
            else {
    ...
    pointLineDist: {
        name: "pointLineDist",
        description: "Try to make distance between a point and a segment `s1` equal to padding.",
        params: [
            { name: "point", type: real2T() },
            { name: "s1", type: shapeT("Line") },
            { name: "padding", type: realT() },
        ],
        body: noWarnFn((point, s1, padding) => {
            return squared(sub(ops.vdist(closestPt_PtSeg(point, [s1.start.contents, s1.end.contents]), point), padding));
        }),
    },
    /**
    ...
    isRegular: {
        ...
        body: noWarnFn((points, closed) => {
            const equilater = constrDictCurves.isEquilateral.body(points, closed).value;
            const equiangular = constrDictCurves.isEquiangular.body(points, closed).value;
...
const _objDictVals = Object.values(objDict);
