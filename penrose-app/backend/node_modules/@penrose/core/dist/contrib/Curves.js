...
import { absVal, add, addN, div, ifCond, lte, maxN, mul, pow, sign, sin, squared, sub, tan, } from "../engine/AutodiffFunctions.js";
 ...
 */
export const equivalued = (x) => {
    const mean = div(addN(x), x.length);
    return addN(x.map((y) => squared(sub(y, mean))));
};
var CurvatureApproximationMode;
(function (CurvatureApproximationMode) {
    CurvatureApproximationMode["FiniteDifferences"] = "FiniteDifferences";
    CurvatureApproximationMode["Angle"] = "Angle";
    CurvatureApproximationMode["SteinerLineSegment"] = "SteinerLineSegment";
    CurvatureApproximationMode["SteinerCorner"] = "SteinerCorner";
    CurvatureApproximationMode["OsculatingCircle"] = "OsculatingCircle";
})(CurvatureApproximationMode || (CurvatureApproximationMode = {}));
/**
 ...
 */
export const curvature = (p1, p2, p3, mode = CurvatureApproximationMode.Angle) => {
    ...
    // $\kappa^A$ from [1]
    if (mode === CurvatureApproximationMode.Angle)
        return angle;
    // $\kappa^B$ from [1]
 ...
 */
export const perimeter = (points, closed) => {
    const sides = consecutiveTuples(points, closed);
    return addN(sides.map(([p1, p2]) => ops.vdist(p1, p2)));
};
/**
 * Returns the signed area enclosed by a polygonal chain given its nodes
 */
export const signedArea = (points, closed) => {
    const sides = consecutiveTuples(points, closed);
    return mul(0.5, addN(sides.map(([p1, p2]) => sub(mul(p1[0], p2[1]), mul(p1[1], p2[0])))));
};
/**
 * Returns the turning number of polygonal chain given its nodes
 */
export const turningNumber = (points, closed) => {
 ...
 */
export const isoperimetricRatio = (points, closed) => {
    return div(squared(perimeter(points, closed)), signedArea(points, closed));
};
/**
 * Returns integral of curvature along the curve
 */
export const totalCurvature = (points, closed, signed = true) => {
    const triples = consecutiveTriples(points, closed);
    if (signed) {
        return addN(triples.map(([p1, p2, p3]) => curvature(p1, p2, p3)));
    }
    return addN(triples.map(([p1, p2, p3]) => absVal(curvature(p1, p2, p3))));
};
/**
 * Returns integral of curvature squared along the curve
 */
export const elasticEnergy = (points, closed) => {
    const triples = consecutiveTriples(points, closed);
    return addN(triples.map(([p1, p2, p3]) => mul(squared(curvature(p1, p2, p3, CurvatureApproximationMode.SteinerLineSegment)), mul(0.5, mul(ops.vdist(p1, p2), ops.vdist(p2, p3))))));
};
/**
 * Returns the sum of all line segment lengths raised to `k`
 */
export const lengthK = (points, closed, k) => {
    const tuples = consecutiveTuples(points, closed);
    return addN(tuples.map(([p1, p2]) => pow(ops.vdist(p1, p2), k)));
};
/**
 * Returns the maximum value of curvature along the curve
 */
export const maxCurvature = (points, closed) => {
    const triples = consecutiveTriples(points, closed);
    return maxN(triples.map(([p1, p2, p3]) => absVal(curvature(p1, p2, p3, CurvatureApproximationMode.OsculatingCircle))));
};
/**
 * Returns integral of curvature raised to `p` along the curve
 */
export const pElasticEnergy = (points, closed, p = 2) => {
    const triples = consecutiveTriples(points, closed);
    return addN(triples.map(([p1, p2, p3]) => mul(pow(curvature(p1, p2, p3, CurvatureApproximationMode.SteinerLineSegment), p), mul(0.5, mul(ops.vdist(p1, p2), ops.vdist(p2, p3))))));
};
/**
 * Inflection energy of an order p
 */
export const inflectionEnergy = (points, closed, p) => {
    const triples = consecutiveTriples(points, closed);
    const curvatures = triples.map(([p1, p2, p3]) => curvature(p1, p2, p3, CurvatureApproximationMode.SteinerLineSegment));
    const tuples = consecutiveTuples(curvatures, closed);
    return addN(tuples.map(([kappa1, kappa2]) => pow(absVal(sub(kappa2, kappa1)), p)));
};
/**
 * Center of mass of a list of vertices
 */
export const centerOfMass = (points) => {
...
export const constrDictCurves = {
    ...
    isLocallyConvex: {
        ...
        description: "The shape should be locally convex (all angles between consecutive edges would have the same sign)",
        params: [
            {
                name: "points",
                type: realNMT(),
        ...
        body: noWarnFn((points, closed) => {
            const triples = consecutiveTriples(points, closed);
            const angles = triples.map(([p1, p2, p3]) => ops.angleFrom(ops.vsub(p2, p1), ops.vsub(p3, p2)));
            const meanSign = sign(addN(angles));
            return addN(angles.map((angle) => ifCond(lte(mul(meanSign, angle), 0), squared(angle), 0)));
        }),
    },
    /**
    ...
    isConvex: {
        ...
        body: noWarnFn((points, closed) => {
            const localPenalty = constrDictCurves.isLocallyConvex.body(points, closed).value;
    ...
    isEquilateral: {
        ...
        body: noWarnFn((points, closed) => {
            ...
            return equivalued(hs.map(([p1, p2]) => ops.vdist(p1, p2)));
        }),
    },
    /**
    ...
    isEquiangular: {
        ...
        body: noWarnFn((points, closed) => {
            ...
            return equivalued(hs.map(([p1, p2, p3]) => ops.angleFrom(ops.vsub(p2, p1), ops.vsub(p3, p2))));
        }),
    },
};
/**
 ...
 */
export const tangentVectors = (points, closed) => {
 ...
 */
export const normalVectors2D = (points, closed) => {
    return tangentVectors(points, closed).map((tangent) => ops.rot90(tangent));
};
/**
 * Returns list of `n` principal normal vectors given a list of `n` points in 3D.
 */
export const principalNormalVectors = (points, closed) => {
    const tangents = tangentVectors(points, closed);
    const principalNormals = [];
 ...
 */
export const binormalVectors = (points, closed) => {
    const tangents = tangentVectors(points, closed);
 ...
 */
export const normalVectors = (points, closed) => {
    const dimension = points[0].length;
    if (dimension === 2) {
        return normalVectors2D(points, closed);
    }
    else
        return principalNormalVectors(points, closed);
};
/**
 * Returns list of `n` curvatures given a list of `n` points.
 */
export const curvatures = (points, closed, mode = CurvatureApproximationMode.FiniteDifferences) => {
    ...
    if (closed) {
        ...
        const lastCurvature = curvature(penultimatePoint, lastPoint, firstPoint, mode);
 ...
 */
export const evoluteCurve = (points, closed) => {
    const normals = principalNormalVectors(points, closed);
    const curvatureList = curvatures(points, closed, CurvatureApproximationMode.FiniteDifferences);
    const evolute = [];
    for (let i = 0; i < points.length; i++) {
        const p = points[i];
        const n = normals[i];
 ...
 */
export const offsetCurve = (points, closed, magnitude) => {
