...
import { add, div, gt, ifCond, lt, lte, max, maxN, min, minN, mul, neg, sqrt, squared, sub, xor, } from "../engine/AutodiffFunctions.js";
import * as BBox from "../engine/BBox.js";
import { computeShapeBbox } from "../shapes/Shapes.js";
import * as ad from "../types/ad.js";
import { noWarn } from "../utils/Util.js";
import { msign, signedDistancePolygon } from "./Functions.js";
import { convexPartitions, overlappingPolygonPoints, overlappingPolygonPointsEllipse, rectangleDifference, rectangleSignedDistance, } from "./Minkowski.js";
import { isLinelike, isPolygonlike, isRectlike, toPt } from "./Utils.js";
/**
 * Return bounding box from any provided shape.
 */
export const bboxFromShape = (shape) => {
    return computeShapeBbox(shape);
};
export const bboxPts = (bbox) => {
    const { topLeft, topRight, bottomLeft, bottomRight } = BBox.corners(bbox);
    return [topRight, topLeft, bottomLeft, bottomRight];
};
export const rectPts = (center, width, height, clockwise = 0) => {
 ...
 */
export const polygonLikePoints = (s) => {
    const t = s.shapeType;
 ...
 */
export const outwardUnitNormal = (lineSegment, insidePoint) => {
    const normal = ops.vnormalize(ops.rot90(ops.vsub(lineSegment[1], lineSegment[0])));
 ...
 */
export const convexPolygonOriginSignedDistance = (p) => {
    const n = p.length;
    ...
    return maxN(segments.map(({ goodLeft, goodRight, edgeSignedDist }, i) => {
        const next = segments[(i + 1) % n];
        return ifCond(goodLeft, ifCond(goodRight, edgeSignedDist, ifCond(next.goodLeft, -Infinity, next.fromStart)), -Infinity);
    }));
};
/**
 ...
 * `-1`).
 */
export const rectLineDist = (rx0, ry0, rx1, ry1, lxs, lys, lxe, lye) => {
    const px = gt(lxs, lxe);
    const py = gt(lys, lye);
...
export const shapeDistance = (s1, s2) => {
    const t1 = s1.shapeType;
    const t2 = s2.shapeType;
    // Same shapes
    if (t1 === "Circle" && t2 === "Circle") {
        return noWarn(shapeDistanceCircles(toPt(s1.center.contents), s1.r.contents, toPt(s2.center.contents), s2.r.contents));
    }
    else if (isRectlike(s1) && isRectlike(s2)) {
        return noWarn(shapeDistanceRects(bboxPts(BBox.bboxFromRectlike(s1)), bboxPts(BBox.bboxFromRectlike(s2))));
    }
    // HACK: text/label-line, mainly to skip convex partitioning
    else if (isRectlike(s1) && t2 === "Line") {
        return noWarn(shapeDistanceRectLine(bboxPts(BBox.bboxFromRectlike(s1)), toPt(s2.start.contents), toPt(s2.end.contents)));
    }
    else if (t1 === "Line" && isRectlike(s2)) {
        return noWarn(shapeDistanceRectLine(bboxPts(BBox.bboxFromRectlike(s2)), toPt(s1.start.contents), toPt(s1.end.contents)));
    }
    else if (isPolygonlike(s1) && isPolygonlike(s2)) {
        return noWarn(shapeDistancePolys(polygonLikePoints(s1), polygonLikePoints(s2)));
    }
    // Rectangle x Circle
    else if (isRectlike(s1) && t2 === "Circle") {
        return noWarn(shapeDistanceRectCircle(bboxPts(BBox.bboxFromRectlike(s1)), toPt(s2.center.contents), s2.r.contents));
    }
    else if (t1 === "Circle" && isRectlike(s2)) {
        return noWarn(shapeDistanceRectCircle(bboxPts(BBox.bboxFromRectlike(s2)), toPt(s1.center.contents), s1.r.contents));
    }
    // Polygon x Ellipse
    else if (isPolygonlike(s1) && t2 === "Ellipse") {
        return noWarn(shapeDistancePolyEllipse(polygonLikePoints(s1), toPt(s2.center.contents), s2.rx.contents, s2.ry.contents));
    }
    else if (t1 === "Ellipse" && isPolygonlike(s2)) {
        return noWarn(shapeDistancePolyEllipse(polygonLikePoints(s2), toPt(s1.center.contents), s1.rx.contents, s1.ry.contents));
    }
    // Circle x Line
    else if (t1 === "Circle" && t2 === "Line") {
        return noWarn(shapeDistanceCircleLine(toPt(s1.center.contents), s1.r.contents, toPt(s2.start.contents), toPt(s2.end.contents)));
    }
    else if (t1 === "Line" && t2 === "Circle") {
        return noWarn(shapeDistanceCircleLine(toPt(s2.center.contents), s2.r.contents, toPt(s1.start.contents), toPt(s1.end.contents)));
    }
    // Line x Line
    else if (t1 === "Line" && t2 === "Line") {
        return noWarn(shapeDistanceLines(toPt(s1.start.contents), toPt(s1.end.contents), toPt(s2.start.contents), toPt(s2.end.contents)));
    }
    else if (t1 === "Polyline" && t2 === "Circle") {
        return noWarn(shapeDistanceCirclePolyline(s2.center.contents, s2.r.contents, s1.points.contents));
    }
    else if (t2 === "Polyline" && t1 === "Circle") {
        return noWarn(shapeDistanceCirclePolyline(s1.center.contents, s1.r.contents, s2.points.contents));
    }
    else if (t1 === "Polyline" && isRectlike(s2)) {
        const bbox = bboxFromShape(s2);
        const corners = BBox.corners(bbox);
        return noWarn(shapeDistanceRectlikePolyline([
            corners.topRight,
            corners.topLeft,
            corners.bottomLeft,
            corners.bottomRight,
    ...
    // Default to axis-aligned bounding boxes
        ...
        return {
            value: shapeDistanceRects(bboxPts(bboxFromShape(s1)), bboxPts(bboxFromShape(s2))),
            warnings: [
                {
...
export const shapeDistanceCircles = (c1, r1, c2, r2) => sub(ops.vdist(c1, c2), add(r1, r2));
export const shapeDistanceRects = (rect1, rect2) => {
...
export const shapeDistanceRectLine = (rect, start, end) => {
    if (rect.length !== 4) {
    ...
    return rectLineDist(sub(cx, halfW), sub(cy, halfH), add(cx, halfW), add(cy, halfH), start[0], start[1], end[0], end[1]);
};
export const shapeDistanceRectlikePolyline = (rect, points) => {
    let dMin = Infinity;
    ...
    for (let i = 0; i < points.length - 1; i++) {
        const a = points[i];
        const b = points[i + 1];
        const d = rectLineDist(bottomLeft[0], bottomLeft[1], topRight[0], topRight[1], a[0], a[1], b[0], b[1]);
        dMin = min(dMin, d);
    }
    return dMin;
};
export const shapeDistancePolys = (pts1, pts2) => overlappingPolygonPoints(pts1, pts2, 0);
export const shapeDistanceRectCircle = (rect, c, r) => {
    if (rect.length !== 4) {
        throw new Error("Expects rect to have four points");
    }
    const bbox = BBox.bboxFromPoints(rect);
    ...
    const x = sub(cx1, cx2);
    const y = sub(cy1, cy2);
    const bottomLeft = [sub(x, halfW), sub(y, halfH)];
    const topRight = [add(x, halfW), add(y, halfH)];
    return sub(rectangleSignedDistance(bottomLeft, topRight), r);
};
export const shapeDistancePolyEllipse = (pts, c, rx, ry) => {
    const cp = convexPartitions(pts);
    return minN(cp.map((p) => overlappingPolygonPointsEllipse(p, c, rx, ry, 0)));
};
export const shapeDistanceCircleLine = (c, r, start, end) => {
    const [a, b] = [start, end];
...
const shapeDistanceCirclePolyline = (c, r, points) => {
...
export const shapeDistanceLines = (start1, end1, start2, end2) => {
    // line endpoints
    const a0 = start1;
    const a1 = end1;
    const b0 = start2;
    ...
    return signedDistancePolygon([toPt(p0), toPt(p1), toPt(p2), toPt(p3)], [0, 0]);
};
