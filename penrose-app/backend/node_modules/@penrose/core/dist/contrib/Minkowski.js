...
import { absVal, add, div, eq, ifCond, max, maxN, min, minN, mul, neg, polyRoots, sqrt, squared, sub, } from "../engine/AutodiffFunctions.js";
import { safe } from "../utils/Util.js";
import { absEllipseToImplicit, ellipsePolynomial, halfPlaneToImplicit, implicitEllipseFunc, implicitHalfPlaneFunc, implicitIntersectionOfEllipsesFunc, } from "./ImplicitShapes.js";
 ...
 */
export const rectangleDifference = ([, tl1, , br1], [, tl2, , br2], padding) => {
    // Prepare coordinates
    ...
    // Return corners
    return [
        [sub(minN(xs), padding), sub(minN(ys), padding)],
 ...
 */
export const halfPlaneSDF = (lineSegment, otherPoints, insidePoint, padding) => {
 ...
 */
const convexPolygonMinkowskiSDFOneSided = (p1, p2, padding) => {
    ...
    const sdfs = sides.map((s) => halfPlaneSDF(s, p2, center, padding));
    return maxN(sdfs);
};
/**
 ...
 * @param padding Padding around the Minkowski sum.
 */
export const convexPolygonMinkowskiSDF = (p1, p2, padding) => {
    return max(convexPolygonMinkowskiSDFOneSided(p1, p2, padding), convexPolygonMinkowskiSDFOneSided(p2, p1, padding));
};
/**
 ...
 */
export const convexPartitions = (p) => {
    if (p.length <= 3) {
        return [p];
    }
    ...
    const pointMap = new Map(p.map((point, i) => {
        const j = 2 * i;
    ...
    return convexPolygons.map((poly) => poly.map((point) => safe(pointMap.get(point), "polygon decomposition unexpectedly created a new point")));
};
/**
 ...
 */
export const overlappingPolygonPoints = (polygonPoints1, polygonPoints2, overlap = 0) => {
    const cp1 = convexPartitions(polygonPoints1);
    const cp2 = convexPartitions(polygonPoints2.map((p) => ops.vneg(p)));
    return maxN(cp1.map((p1) => minN(cp2.map((p2) => convexPolygonMinkowskiSDF(p1, p2, neg(overlap))))));
};
/**
 * Returns the signed distance from a rectangle at the origin.
 */
export const rectangleSignedDistance = (bottomLeft, topRight) => {
    ...
    const e1 = sqrt(add(squared(max(sub(xp, xr), 0)), squared(max(sub(yp, yr), 0))));
 ...
 */
export const containsConvexPolygonPoints = (p1, p2, padding) => {
    const center = ops.vdiv(p1.reduce(ops.vadd), p1.length);
    // Create a list of all sides given by two subsequent vertices
    const sides = Array.from({ length: p1.length }, (_, key) => key).map((i) => [
        p1[i],
    ...
    const sdfs = sides.map((s) => halfPlaneSDF(s, [ops.vneg(p2)], center, padding));
    return maxN(sdfs);
};
/**
 ...
 */
const pointCandidates = (ei, hpi, lambda) => {
 ...
 * @param padding Padding around the Minkowski sum.
 */
export const halfPlaneEllipseSDF = (lineSegment, c, rx, ry, insidePoint, padding) => {
    const hpi = halfPlaneToImplicit(lineSegment, insidePoint, 0);
    const ei = absEllipseToImplicit(c, rx, ry, padding);
    const e = div(add(mul(ei.b, squared(hpi.a)), mul(ei.a, squared(hpi.b))), mul(mul(ei.a, ei.b), mul(4, add(add(mul(ei.x, hpi.a), mul(ei.y, hpi.b)), sub(ei.c, hpi.c)))));
    const ed = sqrt(div(e, add(1, e)));
    const point1 = pointCandidates(ei, hpi, add(1, ed));
    const point2 = pointCandidates(ei, hpi, sub(1, ed));
    const m1 = min(implicitHalfPlaneFunc(hpi, point1[0], point1[1]), implicitHalfPlaneFunc(hpi, point2[0], point2[1]));
    const m2 = max(implicitEllipseFunc(ei, ei.x, ei.y), implicitHalfPlaneFunc(hpi, ei.x, ei.y));
    return min(m1, m2);
};
/**
 ...
 */
export const overlappingPolygonPointsEllipse = (polygonPoints, c, rx, ry, padding) => {
    ...
    const sdfs = sides.map((s) => halfPlaneEllipseSDF(s, c, rx, ry, center, padding));
    return maxN(sdfs);
};
/**
 ...
 * @param lambda Solution to the quadratic formula.
 */
export const pointCandidatesEllipse = (ei1, ei2, lambda) => {
    const x = div(add(mul(ei1.x, ei1.a), mul(lambda, sub(mul(ei2.x, ei2.a), mul(ei1.x, ei1.a)))), add(ei1.a, mul(lambda, sub(ei2.a, ei1.a))));
    const y = div(add(mul(ei1.y, ei1.b), mul(lambda, sub(mul(ei2.y, ei2.b), mul(ei1.y, ei1.b)))), add(ei1.b, mul(lambda, sub(ei2.b, ei1.b))));
    return [x, y];
};
/**
 ...
 */
export const overlappingImplicitEllipses = (ei1, ei2) => {
    const poly = ellipsePolynomial(ei1, ei2);
    const roots = polyRoots(poly).map((r) => ifCond(eq(r, r), r, 0));
    const points = roots.map((root) => pointCandidatesEllipse(ei1, ei2, root));
    const m1 = minN(points.map(([x, y]) => implicitIntersectionOfEllipsesFunc(ei1, ei2, x, y)));
    const m2 = min(implicitIntersectionOfEllipsesFunc(ei1, ei2, ei1.x, ei1.y), implicitIntersectionOfEllipsesFunc(ei1, ei2, ei2.x, ei2.y));
    return min(m1, m2);
};
