...
].map((x) => makeRectangle(context, canvas, {
...
].map((x) => makeCircle(context, canvas, {
    r: floatV(x.r),
    center: vectorV(x.center),
    strokeWidth: floatV(0),
    strokeColor: black(),
...
].map((x) => makeEllipse(context, canvas, {
    rx: floatV(x.rx),
    ry: floatV(x.ry),
    center: vectorV(x.center),
    strokeWidth: floatV(0),
...
export const _polygons = polyPts.map((pts) => makePolygon(context, canvas, {
    points: ptListV(pts),
    scale: floatV(1),
}));
export const _polylines = polyPts.map((pts) => makePolyline(context, canvas, {
    points: ptListV(pts),
    scale: floatV(1),
}));
//# sourceMappingURL=TestShapes.input.js.map