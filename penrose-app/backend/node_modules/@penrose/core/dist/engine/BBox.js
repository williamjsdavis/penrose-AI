...
import { absVal, add, and, div, eq, gt, ifCond, lt, max, min, mul, neg, sqrt, squared, sub, } from "./AutodiffFunctions.js";
/**
 * Input: A width, height, and center.
 * Output: A new BBox.
 */
export const bbox = (width, height, center) => {
    return {
        width,
        height,
        center,
 ...
 */
export const inflate = (b, delta) => {
    return bbox(add(b.width, add(delta, delta)), add(b.height, add(delta, delta)), b.center);
};
/**
 * Input: A BBox.
...
export const bboxFromPoints = (points) => {
    const minCorner = points.reduce((corner, point) => [
        min(corner[0], point[0]),
        min(corner[1], point[1]),
    ]);
    const maxCorner = points.reduce((corner, point) => [
...
export const bboxFromRotatedRect = (center, w, h, clockwise, strokeWidth) => {
    return bboxFromPoints(rectPts(center, add(w, strokeWidth), add(h, strokeWidth), clockwise));
};
export const bboxFromCircle = ({ r, center, strokeWidth, }) => {
    // https://github.com/penrose/penrose/issues/715
    if (!ad.isPt2(center.contents)) {
        throw new Error(`bboxFromCircle expected center to be Pt2, but got length ${center.contents.length}`);
    }
    const diameter = add(mul(2, r.contents), strokeWidth.contents);
    return bbox(diameter, diameter, center.contents);
};
export const bboxFromEllipse = ({ rx, ry, center, strokeWidth, }) => {
    // https://github.com/penrose/penrose/issues/715
    if (!ad.isPt2(center.contents)) {
        throw new Error(`bboxFromEllipse expected center to be Pt2, but got length ${center.contents.length}`);
    }
    const dx = mul(2, rx.contents);
    const dy = mul(2, ry.contents);
    return bbox(add(dx, strokeWidth.contents), add(dy, strokeWidth.contents), center.contents);
};
export const bboxFromRect = ({ width, height, center, strokeWidth, rotation, }) => {
    // https://github.com/penrose/penrose/issues/715
    if (!ad.isPt2(center.contents)) {
        throw new Error(`bboxFromRect expected center to be Pt2, but got length ${center.contents.length}`);
    }
    // rx just rounds the corners, doesn't change the bbox
    return bboxFromRotatedRect(toPt(center.contents), width.contents, height.contents, rotation.contents, strokeWidth.contents);
};
export const bboxFromRectlike = ({ center, width, height, rotation, }) => {
    // https://github.com/penrose/penrose/issues/715
    if (!ad.isPt2(center.contents)) {
        throw new Error(`bboxFromRectlike expected center to be Pt2, but got length ${center.contents.length}`);
    }
    return bboxFromRotatedRect(center.contents, width.contents, height.contents, rotation.contents, 0);
};
export const bboxFromPolygon = ({ points, scale, }) => {
    return bboxFromPoints(points.contents.map((point) => {
        const pt = ops.vmul(scale.contents, point);
        if (ad.isPt2(pt)) {
            return pt;
        }
        else {
            throw new Error(`bboxFromPolygon expected each point to be Pt2, but got length ${point.length}`);
        }
    }));
};
export const bboxFromLinelike = ({ start, end, strokeWidth, }) => {
    // https://github.com/penrose/penrose/issues/715
    if (!ad.isPt2(start.contents)) {
        throw new Error(`bboxFromLinelike expected start to be Pt2, but got length ${start.contents.length}`);
    }
    if (!ad.isPt2(end.contents)) {
        throw new Error(`bboxFromLinelike expected end to be Pt2, but got length ${end.contents.length}`);
    }
    const d = ops.vmul(div(strokeWidth.contents, 2), ops.rot90(ops.vnormalize(ops.vsub(end.contents, start.contents))));
...
export const bboxFromPath = ({ d }) => {
    const p = d.contents;
    if (p.length < 1) {
        throw new Error("bboxFromPath expected pathData to be nonempty");
    }
    if (p[0].cmd !== "M") {
        throw new Error(`bboxFromPath expected first command to be M, but got ${p[0].cmd}`);
    }
    const first = p[0].contents[0];
    if (first.tag !== "CoordV") {
        throw new Error(`bboxFromPath expected first command subpath to be CoordV, but got ${first.tag}`);
    }
    if (!ad.isPt2(first.contents)) {
        throw new Error(`bboxFromPath expected cursor to be Pt2, but got length ${first.contents.length}`);
    }
    let cursor = first.contents;
    let control = cursor; // used by T and S
    const points = [];
    for (const { cmd, contents } of p) {
        ...
        if (cmd === "M") {
            ...
            // eq. 5.1
            const [x1Prime, y1Prime] = ops.vrot(ops.vdiv(ops.vsub(cursor, next.contents), 2), neg(phi));
            // eq. 6.2
            const lambda = add(squared(div(x1Prime, rxPos)), squared(div(y1Prime, ryPos)));
            // eq. 6.3
            ...
            max(1e-18, div(sub(sub(squared(mul(rx, ry)), squared(mul(rx, y1Prime))), squared(mul(ry, x1Prime))), add(squared(mul(rx, y1Prime)), squared(mul(ry, x1Prime))))))), [div(mul(rx, y1Prime), ry), neg(div(mul(ry, x1Prime), rx))]);
            // eq. 5.3
            const [cx, cy] = ops.vadd(ops.vrot(cPrime, phi), ops.vdiv(ops.vadd(cursor, next.contents), 2));
...
export const intersectBbox = (bbox1, bbox2) => {
    ...
    return bbox(ifCond(cond, intersection_w, 0), ifCond(cond, intersection_h, 0), [ifCond(cond, intersection_x, 0), ifCond(cond, intersection_y, 0)]);
};
