...
import { absVal, acos, add, addN, atan2, cos, cosh, div, eq, exp, gt, ifCond, inverse, ln, lt, max, mul, neg, pow, sign, sin, sinh, sqrt, squared, sub, } from "./AutodiffFunctions.js";
...
const children = (x) => {
    ...
    switch (x.tag) {
        ...
        case "PolyRoots": {
            ...
            const derivCoeffs = x.coeffs.map((c, i) => mul(i, c));
            derivCoeffs.shift();
            // the polynomial is assumed monic, so `x.coeffs` doesn't include the
            // coefficient 1 on the highest-degree term
            derivCoeffs.push(n);
            const sensitivities = x.coeffs.map((_, index) => {
                const t = { tag: "Index", index, vec: x }; // a root
                let power = 1;
                const powers = [power];
                for (let i = 1; i < n; i++) {
                    power = mul(power, t);
                ...
                const minusDerivative = neg(addN(zip2(derivCoeffs, powers).map(([c, p]) => mul(c, p))));
 ...
 */
export const makeGraph = (outputs, getKey) => {
    ...
    for (const id of primaryNodes) {
        ...
        const edges = [...graph.outEdges(id)].sort((a, b) => a.j === b.j ? a.e - b.e : a.j - b.j);
 ...
 */
export const primaryGraph = (output, getKey) => makeGraph({ primary: output, secondary: [] }, getKey);
/**
 ...
 */
export const secondaryGraph = (outputs, getKey) => 
...
export const ops = {
     ...
     */
    norm: (c1, c2) => ops.vnorm([c1, c2]),
    /**
     * Return the Euclidean distance between scalars `c1, c2`.
     */
    dist: (c1, c2) => ops.vnorm([c1, c2]),
    /**
    ...
    vnormsq: (v) => {
        ...
        return _.reduce(res, (x, y) => add(x, y), 0);
    ...
    mvmul: (A, v) => {
        ...
        for (let i = 0; i < v.length; i++) {
            ...
            result.push(summands.reduce((x, y) => add(x, y), 0));
        }
        return result;
    },
    /**
    ...
    mmmul: (A, B) => {
        ...
        for (let i = 0; i < A.length; i++) {
            ...
            for (let j = 0; j < A.length; j++) {
                ...
                row.push(summands.reduce((x, y) => add(x, y), 0));
            }
            result.push(row);
        }
        return result;
    ...
    angleFrom: (u, v) => {
        ...
        return atan2(ops.cross2(u, v), // y = |u||v|sin(theta)
        ops.vdot(u, v));
    },
    /**
     ...
     */
    vsum: (v) => {
        return _.reduce(v, (x, y) => add(x, y), 0);
    },
    /**
     * Return `v + c * u`.
     */
...
const getTypeIndex = (kind) => Object.keys(funcTypes).indexOf(kind);
const getParamIndex = (sig, name) => Object.keys(sig.param).indexOf(name);
const builtindex = new Map([...builtins.keys()].map((name, i) => [name, i]));
const getBuiltindex = (name) => safe(builtindex.get(name), "unknown builtin");
const typeSection = (t) => {
    ...
    for (const { param, result } of Object.values(funcTypes)) {
        ...
        for (const typ of Object.values(param))
            t.byte(typ);
        t.int(result.length);
...
const functionSection = (t, numAddends) => {
    t.int(numAddends + 1);
    for (let i = 0; i < numAddends; i++)
        t.int(getTypeIndex("addend"));
    t.int(getTypeIndex("sum"));
};
const exportSection = (t, numAddends) => {
    const numExports = 1;
    t.int(numExports);
...
const modulePrefix = (gradientFunctionSizes) => {
    const numSections = 5;
    ...
    const sumSectionSizes = numSections +
...
const compileUnary = (t, { unop }, param) => {
    switch (unop) {
        case "squared": {
...
const compileBinary = (t, { binop }, left, right) => {
    switch (binop) {
        case "+":
        case "*":
        case "-":
...
const compileNode = (t, node, preds) => {
    switch (node.tag) {
        case "Const": {
            t.byte(wasm.OP.f64.const);
            t.f64(node.val);
...
const getIndex = (locals, id) => {
    const local = safe(locals.indices.get(id), "missing local");
    return (numAddendParams +
        (local.typename === "i32" ? 0 : locals.counts.i32) +
        local.index);
};
const compileGraph = (t, { graph, nodes, gradient, primary, secondary }) => {
    ...
    for (const { id, label: { key }, } of getInputNodes(graph)) {
        t.byte(wasm.OP.local.get);
...
const genBytes = (graphs) => {
    const secondaryKeys = new Map();
    ...
    for (const [k, n] of secondaryKeys) {
        if (n > 1)
            throw Error(`secondary output ${k} is present in ${n} graphs`);
    }
    ...
    if (mod.count.size !== mod.bytes.length)
        throw Error(`allocated ${mod.bytes.length} bytes but used ${mod.count.size}`);
    return mod.bytes;
};
const makeMeta = (graphs) => {
    const offsetInputs = 0;
    const numInputs = Math.max(0, ...graphs.flatMap(({ graph }) => getInputNodes(graph).map(({ label: { key } }) => key + 1)));
...
const makeImports = (memory) => ({
    [importModule]: {
        [importMemoryName]: memory,
        ...Object.fromEntries([...builtins.keys()].map((name, i) => [
            i.toString(36),
            {
                inverse: (x) => 1 / x,
                acos: Math.acos,
...
const getExport = (meta, instance) => {
    ...
    return () => f(meta.offsetInputs, meta.offsetMask, meta.offsetGradient, meta.offsetSecondary, meta.offsetStack);
};
const makeCompiled = (graphs, meta, instance) => {
    const indices = new Map();
 ...
 */
export const genCode = async (...graphs) => {
    const meta = makeMeta(graphs);
    const instance = await WebAssembly.instantiate(await WebAssembly.compile(genBytes(graphs)), makeImports(meta.memory));
 ...
 */
export const genCodeSync = (...graphs) => {
    const meta = makeMeta(graphs);
    const instance = new WebAssembly.Instance(new WebAssembly.Module(genBytes(graphs)), makeImports(meta.memory));
    return makeCompiled(graphs, meta, instance);
};
/** Generate an energy function from the current state (using `ad.Num`s only) */
export const genGradient = async (inputs, objectives, constraints) => {
    ...
    const getKey = (x) => safe(indices.get(x), "missing input");
    ...
    const constrs = constraints.map((x, i) => {
        ...
        return makeGraph({ primary: mul(lambda, fns.toPenalty(x)), secondary }, getKey);
    });
    const graphs = [...objs, ...constrs];
    const meta = makeMeta(graphs);
    const instance = await WebAssembly.instantiate(await WebAssembly.compile(genBytes(graphs)), makeImports(meta.memory));
    const f = getExport(meta, instance);
    return ({ inputMask, objMask, constrMask }, inputs, weight, grad) => {
        if (inputMask.length !== n)
            throw Error(`expected ${n} inputs, got input mask with length ${inputMask.length}`);
        if (objMask.length !== objectives.length)
            throw Error(`expected ${objectives.length} objectives, got objective mask with length ${objMask.length}`);
        if (constrMask.length !== constraints.length)
            throw Error(`expected ${constraints.length} constraints, got constraint mask with length ${constrMask.length}`);
        if (inputs.length !== n)
            throw Error(`expected ${n} inputs, got ${inputs.length}`);
        if (grad.length !== n)
            throw Error(`expected ${n} inputs, got gradient with length ${grad.length}`);
...
const isConverged = (params) => params.optStatus === "EPConverged";
export const problem = async ({ objective, constraints, }) => {
    ...
    const constrs = (constraints ?? []).map((x) => primaryGraph(mul(lambda, fns.toPenalty(x)), getKey));
                ...
                return {
                    ...
                    run: ({ until }) => {
                        ...
                        while (!(stop || isConverged(after))) {
                            after = stepUntil((inputs /*read-only*/, weight, grad /*write-only*/) => {
                                if (inputs.length !== n)
                                    throw Error(`expected ${n} inputs, got ${inputs.length}`);
                                if (grad.length !== n)
                                    throw Error(`expected ${n} inputs, got gradient with length ${grad.length}`);
...
export const compile = async (xs) => {
    const indices = new Map();
    const graph = secondaryGraph(xs, (x) => {
        let i = indices.get(x);
        if (i === undefined) {
            i = indices.size;
            indices.set(x, i);
