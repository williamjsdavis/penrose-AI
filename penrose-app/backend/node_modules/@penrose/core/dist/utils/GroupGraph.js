...
export const makeGroupGraph = (shapes) => {
...
export const getParent = (graph, node) => {
...
export const traverseUp = (graph, node) => {
    const path = [];
    let currNode = node;
    while (currNode) {
        path.push(currNode);
...
export const buildRenderGraphNode = (name, groupGraph, nameShapeMap) => {
    ...
    shape.shapes = shapeListV(buildRenderGraph(childrenNames, groupGraph, nameShapeMap));
    return shape;
};
export const buildRenderGraph = (roots, groupGraph, nameShapeMap) => {
    const orderedRoots = roots.sort((a, b) => groupGraph.node(a) - groupGraph.node(b));
    return orderedRoots.map((root) => buildRenderGraphNode(root, groupGraph, nameShapeMap));
};
//# sourceMappingURL=GroupGraph.js.map