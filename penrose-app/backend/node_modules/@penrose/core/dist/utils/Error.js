...
export const showError = (error) => {
    switch (error.tag) {
        ...
        case "InvalidColorLiteral":
            return `${error.color.contents} (at ${loc(error.color)}) is not a valid color literal. Color literals must be one of the following formats: #RGB, #RGBA, #RRGGBB, #RRGGBBAA.`;
        case "TypeDeclared": {
        ...
        case "VarNotFound": {
            ...
            const msg = `Variable ${variable.value} (at ${loc(variable)}) does not exist.`;
            if (possibleVars) {
        ...
        case "TypeNotFound": {
            ...
            const msg = `Type ${typeName.value} (at ${loc(typeName)}) does not exist.`;
            if (possibleTypes) {
        ...
        case "TypeVarNotFound": {
            return `Type variable ${error.typeVar.name.value} (at ${loc(error.typeVar)}) does not exist.`;
        }
        case "DuplicateName": {
            const { firstDefined, name, location } = error;
            return `Name ${name.value} (at ${loc(location)}) already exists, first declared at ${loc(firstDefined)}.`;
        }
        case "CyclicSubtypes": {
            return `Subtyping relations in this program form a cycle. Cycles of types are:\n${showCycles(error.cycles)}`;
        }
        case "SymmetricTypeMismatch": {
            const { sourceExpr } = error;
            return `The symmetric predicate at ${loc(sourceExpr)} must have arguments all of the same type.`;
        }
        case "SymmetricArgLengthMismatch": {
            const { sourceExpr } = error;
            return `The symmetric predicate at ${loc(sourceExpr)} must only have two arguments.`;
        }
        case "TypeMismatch": {
            const { sourceExpr, sourceType, expectedExpr, expectedType } = error;
            return `The type of the expression at ${loc(sourceExpr)} '${showType(sourceType)}' does not match with the expected type '${showType(expectedType)}' derived from the expression at ${loc(expectedExpr)}.`;
        }
        case "TypeArgLengthMismatch": {
            const { sourceExpr, sourceType, expectedExpr, expectedType } = error;
            return `${expectedType.args.length} arguments expected for type ${expectedType.name.value} (defined at ${loc(expectedExpr)}), but ${sourceType.args.length} arguments were given for ${sourceType.name.value} at ${loc(sourceExpr)} `;
        }
        case "ArgLengthMismatch": {
            const { name, argsGiven, argsExpected, sourceExpr, expectedExpr } = error;
            return `${name.value} expects ${argsExpected.length} arguments (originally defined at ${loc(expectedExpr)}), but was given ${argsGiven.length} arguments instead at ${loc(sourceExpr)}.`;
        }
        case "DeconstructNonconstructor": {
            const { variable, field } = error.deconstructor;
            return `Because ${variable.value} is not bound to a constructor, ${variable.value}.${field.value} (at ${loc(error.deconstructor)}) does not correspond to a field value.`;
        }
        case "UnexpectedExprForNestedPred": {
            const { sourceExpr, sourceType, expectedExpr } = error;
            return `A nested predicate is expected (defined at ${loc(expectedExpr)}), but an expression of '${showType(sourceType)}' type was given at ${loc(sourceExpr)}.`;
        }
        case "InvalidSetIndexingError": {
            const { index, location, suggestions } = error;
            return `Name \`${index}\` (which is used at ${loc(location)}) is not a valid index. Possible indices are: ${suggestions.join(", ")}`;
        }
        case "DuplicateIndexError": {
            const { index, location } = error;
            return `Index variable \`${index}\` has been declared multiple times at ${loc(location)}.`;
        }
        ...
        case "InvalidArithmeticValueError": {
            ...
            return `The expression at ${loc(location)} resulted in the invalid value of ${value}.`;
        }
        case "UnsupportedIndexingError": {
            const { iset } = error;
            return `Indexing on expressions of type ${iset.stmt.tag} (at ${loc(iset)}) is not supported`;
        }
        // ---- BEGIN STYLE ERRORS
        ...
        case "SelectorFieldNotSupported": {
            return `Cannot match on field ${error.name.contents.value}.${error.field.value} (${loc(error.field)}) because matching on fields is not fully supported. Currently, only "label" can be matched in selectors.`;
        }
      ...
      Domain or Style pattern statement has already declared the variable ${error.alias.value}`;
        ...
        case "InvalidGPITypeError": {
            const shapeNames = shapeTypes;
            return `Got invalid GPI type ${error.givenType.value}. Available shape types: ${shapeNames}`;
        }
        case "InvalidGPIPropertyError": {
            return `Got invalid GPI property ${error.givenProperty.value} at ${loc(error.givenProperty)}. Available properties: ${error.expectedProperties}`;
        }
        ...
        case "AssignAccessError": {
            return `Cannot directly assign to or delete an index ${prettyPrintPath(error.path)} of a larger structure (at ${loc(error.path)}).`;
        }
        case "AssignGlobalError": {
            return `Cannot assign to global ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}); instead, just assign to ${error.path.members
                .map((id) => id.value)
                .join(".")} inside the ${error.path.name} namespace.`;
        }
        case "AssignSubstanceError": {
            return `Cannot assign to Substance object ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}).`;
        }
        case "BadElementError": {
            if (error.coll.tag === "CollectionAccess") {
                const preamble = `The collection access (at ${locc("Style", error.coll)}) failed`;
                if (error.index === 0) {
                    return (preamble +
                ...
                return `Wrong element type at index ${error.index} in ${error.coll.tag} (at ${loc(error.coll)}).`;
            }
        }
        case "BadIndexError": {
        ...
        case "BinOpTypeError": {
            return `Unsupported binary operation ${error.expr.op} on types ${error.left} and ${error.right} (at ${loc(error.expr)}).`;
        }
...
canvas {
    ${error.attr} = <my desired ${error.attr}>
        ...
        case "CyclicAssignmentError": {
            const cycleString = error.cycles.map((c) => c.map(({ id, src }) => src === undefined ? id : `${id} (${locc("Style", src)})`));
            return `The Style program contains cyclic variable assignments, where the following variables are defined in cycles:\n${showCycles(cycleString)}`;
        }
        case "DeleteGlobalError": {
            return `Cannot delete global ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}).`;
        }
        case "DeleteSubstanceError": {
            return `Cannot delete Substance object ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}).`;
        }
        case "MissingPathError": {
            return `Could not find ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}).`;
        }
        case "MissingShapeError": {
            return `Expected to find shape already defined to hold property ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}), found nothing.`;
        }
        case "NestedShapeError": {
            return `Cannot define shape (at ${loc(error.expr)}) within another shape.`;
        }
        case "NotCollError": {
        ...
        case "NotShapeError": {
            return `Expected to find shape to hold property ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}), found ${error.what}.`;
        }
        case "NotValueError": {
            return `Expected value (at ${loc(error.expr)}), found ${error.what ?? "shape"}.`;
        }
        case "OutOfBoundsError": {
            return `Indices ${error.indices
                .map((i) => `[${i}]`)
                .join("")} of path ${prettyPrintPath(error.expr)} out of bounds (at ${loc(error.expr)}).`;
        }
        case "PropertyMemberError": {
            return `Cannot assign to member ${prettyPrintResolvedPath(error.path)} of a property (at ${locc("Style", error.path)}).`;
        }
        case "UOpTypeError": {
            return `Unsupported unary operation ${error.expr.op} on type ${error.arg} (at ${loc(error.expr)}).`;
        }
        case "BadShapeParamTypeError": {
            const expectedType = error.expectedType;
            const expectedClause = `expects type ${expectedType}`;
            const doesNotAcceptClause = error.value.tag === "Val"
        ...
        case "RedeclareNamespaceError": {
            return `Namespace ${error.existingNamespace} already exists and is redeclared in ${locc("Style", error.location)}.`;
        }
        ...
        case "ImplicitOverrideWarning": {
            return `Implicitly overriding ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}).`;
        }
        case "NoopDeleteWarning": {
            return `Deleting nonexistent ${prettyPrintResolvedPath(error.path)} (at ${locc("Style", error.path)}).`;
        }
        case "LayerCycleWarning": {
            return `Cycles detected in layering order: ${error.cycles
                .map((c) => c.join(", "))
                .join("; ")}. The system approximated a global layering order instead: ${error.approxOrdering.join(", ")}`;
        }
        case "ShapeBelongsToMultipleGroups": {
            return `Shape ${error.shape} belongs to multiple groups: ${error.groups.join(", ")}`;
        }
        ...
        case "BBoxApproximationWarning": {
            ...
            const loc = topItem.location === undefined
                ? ""
                : `(at ${locc("Style", topItem.location)}) `;
            const topStr = `Function call ${topItem.signature} ${loc}uses bounding box approximations`;
            const restStrs = rest.map((item) => `- Function call ${item.signature} uses bounding box approximations`);
...
const showCycles = (cycles) => {
    // repeats the cycle start again
    const pathString = (path) => [...path, path[0]].join(" -> ");
    return cycles.map(pathString).join("\n");
};
export const cyclicSubtypes = (cycles) => ({
    tag: "CyclicSubtypes",
    cycles,
});
// action constructors for error
export const duplicateName = (name, location, firstDefined) => ({
    tag: "DuplicateName",
    name,
    location,
    firstDefined,
});
export const typeNotFound = (typeName, possibleTypes) => ({
    tag: "TypeNotFound",
    typeName,
    possibleTypes,
});
export const varNotFound = (variable, possibleVars) => ({
    tag: "VarNotFound",
    variable,
    possibleVars,
});
export const symmetricTypeMismatch = (sourceExpr) => ({
    tag: "SymmetricTypeMismatch",
    sourceExpr,
});
export const symmetricArgLengthMismatch = (sourceExpr) => ({
    tag: "SymmetricArgLengthMismatch",
    sourceExpr,
});
export const typeMismatch = (sourceType, expectedType, sourceExpr, expectedExpr) => ({
    tag: "TypeMismatch",
    sourceExpr,
    sourceType,
    expectedExpr,
...
export const unexpectedExprForNestedPred = (sourceType, sourceExpr, expectedExpr) => ({
    tag: "UnexpectedExprForNestedPred",
    sourceType,
    sourceExpr,
    expectedExpr,
});
export const argLengthMismatch = (name, argsGiven, argsExpected, sourceExpr, expectedExpr) => ({
...
export const typeArgLengthMismatch = (sourceType, expectedType, sourceExpr, expectedExpr) => ({
...
export const selectorFieldNotSupported = (name, field) => ({
    tag: "SelectorFieldNotSupported",
    name,
    field,
});
export const deconstructNonconstructor = (deconstructor) => ({
...
export const parseError = (message, location, fileType) => ({
    tag: "ParseError",
    message,
    location,
    fileType,
});
export const invalidColorLiteral = (color) => ({
    tag: "InvalidColorLiteral",
    color,
});
export const badShapeParamTypeError = (path, value, expectedType, passthrough) => ({
...
export const badArgumentTypeError = (funcName, funcArg, provided) => ({
    tag: "BadArgumentTypeError",
    funcName,
    funcArg,
    provided,
});
export const missingArgumentError = (funcName, funcArg, funcLocation) => ({
    tag: "MissingArgumentError",
    funcName,
    funcArg,
    funcLocation,
});
export const tooManyArgumentsError = (func, funcLocation, numProvided) => ({
    tag: "TooManyArgumentsError",
    func,
    funcLocation,
    numProvided,
});
export const functionInternalError = (func, location, message) => ({
    tag: "FunctionInternalError",
    func,
    location,
    message,
});
export const redeclareNamespaceError = (existingNamespace, location) => ({
    tag: "RedeclareNamespaceError",
    existingNamespace,
    location,
});
export const notSubstanceCollectionError = (name, location) => ({
    tag: "NotSubstanceCollectionError",
    name,
    location,
});
export const notStyleVariableError = (name, location) => ({
    tag: "NotStyleVariableError",
    name,
    location,
});
export const nanError = (message, lastState) => ({
    tag: "NaNError",
    message,
    lastState,
});
// If there are multiple errors, just return the tag of the first one
export const toStyleErrors = (errors) => {
    if (!errors.length) {
...
export const genericStyleError = (messages) => ({
    errorType: "StyleError",
    tag: "GenericStyleError",
    messages: messages.map(showError),
});
/* name stands for "`loc` concrete" */
const locc = (nodeType, node) => {
    return `line ${node.start.line}, column ${node.start.col + 1} of ${nodeType} program`;
};
...
// #endregion
// #region Either monad
export const every = (...results) => results.reduce((currentResult, nextResult) => and(nextResult, currentResult), results[0]);
export const safeChain = (itemList, func, initialResult) => itemList.reduce((currentResult, nextItem) => andThen((res) => func(nextItem, res), currentResult), initialResult);
