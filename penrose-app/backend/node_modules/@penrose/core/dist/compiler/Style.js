...
import { makeCanvas, uniform, } from "../shapes/Samplers.js";
import { isShapeType, sampleShape, } from "../shapes/Shapes.js";
import { all, andThen, badShapeParamTypeError, err, invalidColorLiteral, isErr, notStyleVariableError, notSubstanceCollectionError, ok, parseError, redeclareNamespaceError, safeChain, selectorFieldNotSupported, toStyleErrors, } from "../utils/Error.js";
import Graph from "../utils/Graph.js";
import { buildRenderGraph, findOrderedRoots, makeGroupGraph, traverseUp, } from "../utils/GroupGraph.js";
import Heap from "../utils/Heap.js";
import { boolV, cartesianProduct, colorV, floatV, getAdValueAsString, hexToRgba, isKeyOf, listV, llistV, matrixV, prettyPrintResolvedPath, ptListV, resolveRhsName, shapeListV, strV, tupV, val, vectorV, zip2, } from "../utils/Util.js";
...
const log = consola
    .create({ level: consola.LogLevel.Warn })
...
//#endregion
//#region utils
const dummyId = (name) => dummyIdentifier(name, "SyntheticStyle");
export function numbered(xs) {
    return zip2(xs, _.range(xs.length));
}
const safeContentsList = (x) => x ? x.contents : [];
const toString = (x) => x.contents.value;
const oneErr = (err) => {
...
const addDiags = ({ errors, warnings }, x) => ({
    ...x,
    diagnostics: {
        ...x.diagnostics,
...
// `checkDeclPattern`
const checkDeclPatternAndMakeEnv = (varEnv, selEnv, stmt) => {
    ...
    const varName = bVar.contents.value;
...
const checkDeclPatternsAndMakeEnv = (varEnv, selEnv, decls) => {
 ...
 */
const getDomainKeywords = (varEnv) => {
    const keyWordMaps = [
        varEnv.types,
        varEnv.functions,
        varEnv.predicates,
...
// Judgment 5. G |- [|S_r] ok
const checkRelPatterns = (varEnv, selEnv, rels) => {
    return _.flatMap(rels, (rel) => checkRelPattern(varEnv, selEnv, rel));
};
const toSubstanceType = (styT) => {
...
const mergeMapping = (varProgTypeMap, varEnv, [varName, styType]) => {
    const res = varProgTypeMap[varName];
    if (!res) {
        throw Error("var has no binding form?");
    }
    const [, bindingForm] = res;
    const vars = varEnv.vars.set(bindingForm.contents.value, toSubstanceType(styType));
...
// Judgment 1. G || g |-> ...
const mergeEnv = (varEnv, selEnv) => {
    return Object.entries(selEnv.sTypeVarMap).reduce((acc, curr) => mergeMapping(selEnv.varProgTypeMap, acc, curr), varEnv);
};
const checkSelector = (varEnv, sel) => {
    ...
    const emptyVarsEnv = { ...varEnv, vars: im.Map(), varIDs: [] };
    const relErrs = checkRelPatterns(mergeEnv(emptyVarsEnv, selEnv_decls), selEnv_decls, safeContentsList(sel.where));
...
const checkCollector = (varEnv, col) => {
    const selEnv_afterHead = checkDeclPatternAndMakeEnv(varEnv, initSelEnv(), col.head);
    const selEnv_afterWith = checkDeclPatternsAndMakeEnv(varEnv, selEnv_afterHead, safeContentsList(col.with));
    const selEnv_afterGroupby = checkDeclPatternsAndMakeEnv(varEnv, selEnv_afterWith, safeContentsList(col.foreach));
    const emptyVarsEnv = { ...varEnv, vars: im.Map(), varIDs: [] };
    const relErrs = checkRelPatterns(mergeEnv(emptyVarsEnv, selEnv_afterGroupby), selEnv_afterGroupby, safeContentsList(col.where));
    return {
        ...selEnv_afterGroupby,
...
const checkHeader = (varEnv, header) => {
    switch (header.tag) {
        case "Selector": {
...
export const fullSubst = (selEnv, subst) => {
    // Check if a variable is a style variable, not a substance one
    const isStyVar = (e) => selEnv.varProgTypeMap[e][0].tag === "StyProgT";
...
export const uniqueKeysAndVals = (subst) => {
 ...
 */
const getSubPredAliasInstanceName = (pred) => {
...
const substituteBform = (lv, subst, bform) => {
    // theta(B) = ...
    switch (bform.tag) {
        case "SubVar": {
...
const substituteExpr = (subst, expr) => {
    // theta(B) = ...
    switch (expr.tag) {
        case "SEBind": {
            return {
...
const substitutePredArg = (subst, predArg) => {
    switch (predArg.tag) {
        case "RelPred": {
            return {
                ...predArg,
...
// theta(|S_r) = ...
export const substituteRel = (subst, rel) => {
    switch (rel.tag) {
        case "RelBind": {
    ...
    switch (e.tag) {
        ...
        case "SEFuncOrValCons": {
            ...
            if (env.constructors.has(e.name.value)) {
                ...
                throw new Error(`Style internal error: expected '${e.name.value}' to be either a constructor or function, but was not found`);
            }
            const res = {
                ...e,
                tag,
...
const argsEq = (a1, a2, env) => {
    if (a1.tag === "ApplyPredicate" && a2.tag === "ApplyPredicate") {
        ...
        return subExprsEq(a1, a2, env);
    }
    else
        return false; // they are different types
};
const subFnsEq = (p1, p2, env) => {
    ...
    if (zip2(p1.args, p2.args).every(([a1, a2]) => argsEq(a1, a2, env))) {
        ...
        if (predicateDecl && predicateDecl.symmetric) {
            return zip2(p1.args, [p2.args[1], p2.args[0]]).every(([a1, a2]) => argsEq(a1, a2, env));
        }
        else {
            return false;
        }
 ...
 */
const deduplicate = (typeEnv, subEnv, subProg, rels, pSubsts) => {
    const initSubsts = im.List();
    const initMatches = im.Set();
    const [, goodSubsts] = pSubsts.reduce(([currMatches, currSubsts], [subst, matchedSubStmts]) => {
        const record = im.Record({
            rels: matchedSubStmts,
            substTargets: im.Set(Object.values(subst)),
        })();
...
const merge = (s1, s2) => {
    ...
    const result = cartesianProduct(s1Arr, s2Arr, ([aSubst], [bSubst]) => {
 ...
 */
const consistentSubsts = (a, b) => {
    const aKeys = im.Set(Object.keys(a));
    const bKeys = im.Set(Object.keys(b));
...
const typesMatched = (varEnv, substanceType, styleType) => {
...
const matchDeclLine = (varEnv, line, decl) => {
    if (line.tag === "Decl") {
...
const matchDecl = (varEnv, subProg, decl) => {
    const initDSubsts = im.List();
 ...
 */
const matchStyArgToSubArg = (styTypeMap, subTypeMap, varEnv, styArg, subArg) => {
    ...
    if (styArg.tag === "RelPred" && subArg.tag === "ApplyPredicate") {
        return matchStyApplyToSubApply(styTypeMap, subTypeMap, varEnv, styArg, subArg);
    }
    if (subArg.tag === "ApplyConstructor" &&
        (styArg.tag === "SEValCons" || styArg.tag === "SEFuncOrValCons")) {
        return matchStyApplyToSubApply(styTypeMap, subTypeMap, varEnv, styArg, subArg);
    }
    if (subArg.tag === "ApplyFunction" &&
        (styArg.tag === "SEValCons" || styArg.tag === "SEFuncOrValCons")) {
        return matchStyApplyToSubApply(styTypeMap, subTypeMap, varEnv, styArg, subArg);
    }
    return [];
 ...
 */
const matchStyArgsToSubArgs = (styTypeMap, subTypeMap, varEnv, styArgs, subArgs) => {
    const stySubArgPairs = zip2(styArgs, subArgs);
    const substsForEachArg = stySubArgPairs.map(([styArg, subArg]) => {
        const argSubsts = matchStyArgToSubArg(styTypeMap, subTypeMap, varEnv, styArg, subArg);
        return argSubsts;
    });
    ...
    if (first !== undefined) {
        const substs = substsForEachArg.reduce((currSubsts, substsForArg) => {
            return cartesianProduct(currSubsts, substsForArg, (aSubst, bSubst) => consistentSubsts(aSubst, bSubst), (aSubst, bSubst) => combine(aSubst, bSubst));
        }, first);
        return substs;
    }
    else {
 ...
 */
const matchStyApplyToSubApply = (styTypeMap, subTypeMap, varEnv, styRel, subRel) => {
    ...
    if (styRel.tag === "RelPred" && subRel.tag === "ApplyPredicate") {
        ...
        const rSubstOriginal = matchStyArgsToSubArgs(styTypeMap, subTypeMap, varEnv, styRel.args, subRel.args);
        ...
        if (predicateDecl && predicateDecl.symmetric) {
            ...
            rSubstSymmetric = matchStyArgsToSubArgs(styTypeMap, subTypeMap, varEnv, flippedStyArgs, subRel.args);
        }
        const rSubsts = [...rSubstOriginal];
    ...
    if ((subRel.tag === "ApplyConstructor" &&
        ...
        const rSubst = matchStyArgsToSubArgs(styTypeMap, subTypeMap, varEnv, styRel.args, subRel.args);
        return rSubst;
    }
    return [];
};
 ...
 */
const matchRelField = (styTypeMap, subTypeMap, varEnv, subEnv, rel, subDecl) => {
...
const getStyPredOrFuncOrConsArgNames = (arg) => {
    if (arg.tag === "RelPred") {
        return getStyRelArgNames(arg);
    }
    else if (arg.tag === "SEBind") {
        return im.Set().add(toString(arg.contents));
    }
    else {
...
const getStyRelArgNames = (rel) => {
    const initArgNames = im.Set();
    if (rel.tag === "RelPred") {
 ...
 */
const matchStyRelToSubRels = (styTypeMap, subTypeMap, varEnv, subEnv, rel, subProg) => {
    const initUsedStyVars = im.Set();
    const initRSubsts = im.List();
    if (rel.tag === "RelPred") {
        const styPred = rel;
        const newRSubsts = subProg.statements.reduce((rSubsts, statement) => {
            ...
            const rSubstsForPred = matchStyApplyToSubApply(styTypeMap, subTypeMap, varEnv, styPred, statement);
            return rSubstsForPred.reduce((rSubsts, rSubstForPred) => {
                return rSubsts.push([
                    rSubstForPred,
                    im.Set().add(statement),
                ]);
            }, rSubsts);
        }, initRSubsts);
        ...
        const newRSubsts = subProg.statements.reduce((rSubsts, statement) => {
            ...
            const rSubstsForExpr = matchStyApplyToSubApply(styTypeMap, subTypeMap, varEnv, styBindedExpr, subBindedExpr);
        ...
        const newRSubsts = subProg.statements.reduce((rSubsts, statement) => {
            if (statement.tag === "Decl") {
                const rSubst = matchRelField(styTypeMap, subTypeMap, varEnv, subEnv, rel, statement);
                if (rSubst === undefined) {
                    return rSubsts;
                }
                else {
                    return rSubsts.push([rSubst, im.Set()]);
                }
            }
            else {
                return rSubsts;
 ...
 */
const makeListRSubstsForStyleRels = (styTypeMap, subTypeMap, varEnv, subEnv, rels, subProg) => {
    const initUsedStyVars = im.Set();
    const initListRSubsts = im.List();
    const [newUsedStyVars, newListRSubsts] = rels.reduce(([usedStyVars, listRSubsts], rel) => {
        const [relUsedStyVars, relRSubsts] = matchStyRelToSubRels(styTypeMap, subTypeMap, varEnv, subEnv, rel, subProg);
 ...
 */
const makePotentialSubsts = (varEnv, selEnv, subEnv, subProg, decls, rels) => {
    const subTypeMap = subProg.statements.reduce((result, statement) => {
        if (statement.tag === "Decl") {
    ...
    const styTypeMap = selEnv.sTypeVarMap;
    const [usedStyVars, listRSubsts] = makeListRSubstsForStyleRels(styTypeMap, subTypeMap, varEnv, subEnv, rels, subProg);
    ...
    const listPSubsts = decls.reduce((currListPSubsts, decl) => {
        if (usedStyVars.includes(decl.id.contents.value)) {
            ...
            return currListPSubsts.push(pSubsts.map((pSubst) => [pSubst, im.Set()]));
        }
    }, listRSubsts);
...
const getSubsts = (varEnv, subEnv, selEnv, subProg, header) => {
    const decls = getDecls(header);
    const rels = safeContentsList(header.where);
    const rawSubsts = makePotentialSubsts(varEnv, selEnv, subEnv, subProg, decls, rels);
...
const collectSubsts = (substs, toCollect, collectInto, groupbys) => {
...
const findSubstsSel = (varEnv, subEnv, subProg, [header, selEnv]) => {
    if (header.tag === "Selector") {
...
const updateExpr = (path, assignment, errTagGlobal, errTagSubstance, 
    ...
    switch (path.tag) {
        case "Global": {
            if (path.members.length < 1) {
                ...
                return addDiags(oneErr({ tag: "PropertyMemberError", path }), assignment);
            }
...
const processExpr = (context, expr) => {
    if (expr.tag !== "GPIDecl") {
    ...
    const res = safeChain(expr.properties, ({ name, value }, m) => {
...
const insertExpr = (block, path, expr, assignment) => updateExpr(path, assignment, "AssignGlobalError", "AssignSubstanceError", (field, prop, fielded) => {
    const warns = [];
    if (prop === undefined) {
...
const deleteExpr = (path, assignment) => updateExpr(path, assignment, "DeleteGlobalError", "DeleteSubstanceError", (field, prop, fielded) => {
    if (prop === undefined) {
        return ok({
...
const resolveLhsName = ({ block, subst }, assignment, name) => {
...
const resolveLhsPath = (block, assignment, path) => {
...
const processStmt = (block, index, stmt, assignment) => {
    switch (stmt.tag) {
        case "PathAssign": {
        ...
        case "Override": {
            ...
            return insertExpr(block, path.value, stmt.value, deleteExpr(path.value, assignment));
        }
        case "Delete": {
        ...
        case "AnonAssign": {
            ...
            return insertExpr(block, {
                ...range,
                tag: "Local",
                block: block.block,
                name: `$${ANON_KEYWORD}_${index}`,
                members: [],
            }, stmt.contents, assignment);
        }
    }
};
const blockId = (blockIndex, substIndex, header) => {
    switch (header.tag) {
        case "Selector":
        case "Collector": {
...
const makeFakeIntPathAssign = (name, value) => {
    return {
        tag: "PathAssign",
        nodeType: "Style",
        type: undefined,
...
const processBlock = (varEnv, subEnv, blockIndex, hb, assignment) => {
    ...
    return substs.reduce((assignment, subst, substIndex) => {
        ...
        const withLocals = { ...assignment, locals: im.Map() };
        if (block.tag === "NamespaceId") {
            if (withLocals.globals.has(block.contents)) {
                // if the namespace exists, throw an error
                withLocals.diagnostics.errors = errors.push(redeclareNamespaceError(block.contents, {
        ...
        const matchTotalAssignment = makeFakeIntPathAssign("match_total", substs.length);
        ...
        const { diagnostics, globals, unnamed, substances, locals } = augmentedStatements.reduce((assignment, stmt, stmtIndex) => processStmt({ block, subst }, stmtIndex, stmt, assignment), withLocals);
        switch (block.tag) {
            case "LocalVarId": {
                return {
                    diagnostics,
...
export const buildAssignment = (varEnv, subEnv, styProg) => {
    ...
    const assignment = {
        ...
        globals: im.Map(),
        unnamed: im.Map(),
        substances: subEnv.labels.map((label) => im.Map([
            [
                LABEL_FIELD,
                {
                    ...range,
                    ...
                    expr: {
                        context: {
                            block: { tag: "NamespaceId", contents: "" },
    ...
    return styProg.items.reduce((assignment, item, index) => item.tag === "HeaderBlock"
        ? processBlock(varEnv, subEnv, index, item, assignment)
        : assignment, assignment);
};
//#endregion
//#region second pass
const findPathsExpr = (expr, context) => {
    switch (expr.tag) {
        case "BinOp": {
        ...
        case "GPIDecl": {
            return expr.properties.flatMap((prop) => findPathsExpr(prop.value, context));
        }
        case "Layering": {
        ...
        case "CollectionAccess": {
            ...
            if (context.subst.tag === "CollectionSubst" &&
                context.subst.collName === name) {
                const paths = context.subst.collContent.map((subVar) => ({
                    ...expr,
                    tag: "Path",
                    name: {
                        ...expr.name,
...
const findPathsWithContext = ({ context, expr, }) => findPathsExpr(expr, context).map((p) => ({ context, expr: p }));
...
const gatherField = (graph, lhs, rhs) => {
    switch (rhs.tag) {
        case "ShapeSource": {
...
export const gatherDependencies = (assignment) => {
...
//#endregion
//#region third pass
export const internalMissingPathError = (path) => Error(`Style internal error: could not find path ${path}`);
const evalExprs = (mut, canvas, stages, context, args, trans) => all(args.map((expr) => {
    return evalExpr(mut, canvas, stages, { context, expr }, trans);
})).mapErr(flatErrs);
const evalVals = (mut, canvas, stages, context, args, trans) => evalExprs(mut, canvas, stages, context, args, trans).andThen((argVals) => all(argVals.map((argVal, i) => {
    switch (argVal.tag) {
        case "ShapeVal": {
...
const evalBinOpScalars = (error, op, left, right) => {
    switch (op) {
        case "BPlus": {
...
const evalBinOpVectors = (error, op, left, right) => {
    switch (op) {
        case "BPlus": {
...
const evalBinOpScalarVector = (error, op, left, right) => {
    switch (op) {
        case "Multiply": {
...
const evalBinOpVectorScalar = (error, op, left, right) => {
    switch (op) {
        case "Multiply": {
...
const evalBinOpScalarMatrix = (error, op, left, right) => {
    switch (op) {
        case "Multiply": {
...
const evalBinOpMatrixScalar = (error, op, left, right) => {
    switch (op) {
        case "Multiply": {
...
const evalBinOpMatrixVector = (error, op, left, right) => {
    switch (op) {
        case "Multiply": {
...
const evalBinOpVectorMatrix = (error, op, left, right) => {
    switch (op) {
        case "Multiply": {
...
const evalBinOpMatrixMatrix = (error, op, left, right) => {
    switch (op) {
        case "BPlus": {
...
const evalBinOpStrings = (error, op, left, right) => {
    switch (op) {
        case "BPlus": {
...
const evalBinOp = (expr, left, right) => {
    const error = {
        tag: "BinOpTypeError",
        expr,
        left: left.tag,
    ...
    if (left.tag === "FloatV" && right.tag === "FloatV") {
        ...
        return evalBinOpVectors(error, expr.op, left.contents, right.contents).map(vectorV);
    }
    else if (left.tag === "FloatV" && right.tag === "VectorV") {
        return evalBinOpScalarVector(error, expr.op, left.contents, right.contents).map(vectorV);
    }
    else if (left.tag === "VectorV" && right.tag === "FloatV") {
        return evalBinOpVectorScalar(error, expr.op, left.contents, right.contents).map(vectorV);
    }
    else if (left.tag === "FloatV" && right.tag === "MatrixV") {
        return evalBinOpScalarMatrix(error, expr.op, left.contents, right.contents).map(matrixV);
    }
    else if (left.tag === "MatrixV" && right.tag === "FloatV") {
        return evalBinOpMatrixScalar(error, expr.op, left.contents, right.contents).map(matrixV);
    }
    else if (left.tag === "MatrixV" && right.tag === "VectorV") {
        return evalBinOpMatrixVector(error, expr.op, left.contents, right.contents).map(vectorV);
    }
    else if (left.tag === "VectorV" && right.tag === "MatrixV") {
        return evalBinOpVectorMatrix(error, expr.op, left.contents, right.contents).map(vectorV);
    }
    else if (left.tag === "MatrixV" && right.tag === "MatrixV") {
        return evalBinOpMatrixMatrix(error, expr.op, left.contents, right.contents).map(matrixV);
    }
    else if (left.tag === "StrV" && right.tag === "StrV") {
        return evalBinOpStrings(error, expr.op, left.contents, right.contents).map(strV);
    }
    else {
        return err(error);
    }
};
const eval1D = (coll, first, rest) => {
...
const eval2D = (coll, first, rest) => {
    ...
    switch (coll.tag) {
        ...
        case "CollectionAccess": {
            if (first.tag === "ListV")
                return ok(llistV(elems));
            else if (first.tag === "TupV")
                return ok(ptListV(elems));
            else
                return ok(matrixV(elems));
        }
    }
};
const evalShapeList = (coll, first, rest) => {
    const elems = [first];
    for (const v of rest) {
        if (v.tag === "ShapeVal") {
...
const evalListOrVector = (mut, canvas, stages, context, coll, trans) => {
    return evalExprs(mut, canvas, stages, context, coll.contents, trans).andThen((argVals) => {
        if (argVals.length === 0) {
            switch (coll.tag) {
                case "List": {
                    return ok(listV([]));
...
const isValidIndex = (a, i) => Number.isInteger(i) && 0 <= i && i < a.length;
const evalAccess = (expr, coll, indices) => {
    switch (coll.tag) {
        case "ListV":
        case "TupV":
        case "VectorV": {
...
const evalExpr = (mut, canvas, layoutStages, { context, expr }, trans) => {
    switch (expr.tag) {
        case "BinOp": {
            return evalVals(mut, canvas, layoutStages, context, [expr.left, expr.right], trans).andThen(([left, right]) => {
        ...
        case "CompApp": {
            const args = evalExprs(mut, canvas, layoutStages, context, expr.args, trans);
            if (args.isErr()) {
            ...
            if (!isKeyOf(name.value, compDict)) {
                return err(oneErr({ tag: "InvalidFunctionNameError", givenName: name }));
            }
            const f = compDict[name.value];
            const x = callCompFunc(f, { start, end }, mut, argsWithSourceLoc);
            if (x.isErr())
                return err(oneErr(x.error));
        ...
        case "List":
        case "Vector": {
            return evalListOrVector(mut, canvas, layoutStages, context, expr, trans).map(val);
        }
        case "Path": {
            ...
            const res = all(expr.indices.map((e) => evalExpr(mut, canvas, layoutStages, { context, expr: e }, trans).andThen((i) => {
                if (i.tag === "ShapeVal") {
        ...
        case "Tuple": {
            return evalVals(mut, canvas, layoutStages, context, expr.contents, trans).andThen(([left, right]) => {
                if (left.tag !== "FloatV") {
        ...
        case "UOp": {
            return evalExpr(mut, canvas, layoutStages, { context, expr: expr.arg }, trans).andThen((argVal) => {
        ...
        case "Vary": {
            const { exclude } = expr;
            const stages = stageExpr(layoutStages, exclude, expr.stages.map((s) => s.value));
            return ok(val(floatV(mut.makeInput({
        ...
        case "CollectionAccess": {
            ...
            if (subst.tag === "CollectionSubst" && name.value === subst.collName) {
                ...
                return err(oneErr(notSubstanceCollectionError(name.value, {
                    start: expr.start,
                    end: expr.end,
                })));
            }
...
const evalNumberOf = (subst, arg, loc) => {
...
const evalNameOf = (subst, arg, loc) => {
...
const collectIntoVal = (coll, expr) => {
    if (coll.length === 0) {
        return ok(vectorV([]));
    }
    ...
    if (first.tag === "ShapeVal") {
        ...
        else if (first.contents.tag === "VectorV" ||
...
const stageExpr = (overallStages, excludeFlag, stageList) => {
    if (excludeFlag) {
...
const extractObjConstrBody = (body) => {
    if (body.tag === "InlineComparison") {
        const mapInlineOpToFunctionName = (op) => {
            switch (op) {
                case "<":
                    return "lessThan";
                case "==":
...
const translateExpr = (mut, canvas, layoutStages, path, e, trans) => {
    switch (e.expr.tag) {
        case "BinOp":
        case "BoolLit":
        case "ColorLit":
        ...
        case "ConstrFn": {
            ...
            const args = evalExprs(mut, canvas, layoutStages, e.context, argExprs, trans);
            if (args.isErr()) {
            ...
            if (!isKeyOf(fname, constrDict)) {
                return addDiags(oneErr({ tag: "InvalidConstraintNameError", givenName: name }), trans);
            }
            const output = callObjConstrFunc(constrDict[fname], { start: e.expr.start, end: e.expr.end }, argsWithSourceLoc);
            if (output.isErr()) {
                return addDiags(oneErr(output.error), trans);
            }
            const { value, warnings } = output.value;
            const optStages = stageExpr(layoutStages, exclude, stages.map((s) => s.value));
            return {
                ...trans,
                diagnostics: {
                    ...trans.diagnostics,
        ...
        case "ObjFn": {
            ...
            if (!isKeyOf(fname, objDict)) {
                return addDiags(oneErr({ tag: "InvalidObjectiveNameError", givenName: name }), trans);
            }
            const optStages = stageExpr(layoutStages, exclude, stages.map((s) => s.value));
            const output = callObjConstrFunc(objDict[fname], { start: e.expr.start, end: e.expr.end }, argsWithSourceLoc);
            if (output.isErr()) {
                return addDiags(oneErr(output.error), trans);
            }
            const { value, warnings } = output.value;
        ...
        case "Layering": {
            ...
            const leftPp = prettyPrintResolvedPath(resolveRhsPath({ context: context, expr: expr.left }));
            const leftResolved = evalExpr(mut, canvas, layoutStages, { context, expr: expr.left }, trans);
            const rightListPp = expr.right.map((r) => prettyPrintResolvedPath(resolveRhsPath({ context: context, expr: r })));
            const rightListResolved = evalExprs(mut, canvas, layoutStages, context, expr.right, trans);
            if (leftResolved.isErr()) {
                return addDiags(leftResolved.error, trans);
            }
            if (leftResolved.value.tag !== "ShapeVal") {
                return addDiags(oneErr({
            ...
            const layeringRelations = rightListPp.map((r) => {
...
const evalGPI = (path, shapeType, trans) => {
    return checkShape(shapeType, path, trans);
};
export const translate = (mut, canvas, stages, graph, warnings) => {
    let symbols = im.Map();
    ...
    const cycles = graph.findCycles().map((cycle) => cycle.map((id) => {
        const e = graph.node(id);
        return {
            id,
            src: e === undefined || typeof e === "string"
                ? undefined
...
//#endregion
//#region group graph
export const checkGroupGraph = (groupGraph) => {
    const warnings = [];
...
export const processLayering = ({ below, above }, groupGraph, layerGraph) => {
...
export const computeLayerOrdering = (allGPINames, partialOrderings, groupGraph) => {
    const layerGraph = new Graph();
    allGPINames.map((node) => {
        layerGraph.setNode(node, undefined);
    });
    partialOrderings.forEach(({ below, above }) => {
    ...
    if (cycles.length === 0) {
        const shapeOrdering = layerGraph.topsort();
...
export const getCanvasDim = (attr, graph) => {
    const i = `canvas.${attr}`;
    if (!graph.hasNode(i))
...
export const getLayoutStages = (prog) => {
    const layoutStmts = prog.items.filter((i) => i.tag === "LayoutStages");
...
const getShapesList = ({ symbols }, shapeOrdering) => {
...
const onCanvases = (canvas, shapes) => {
    const fns = [];
    for (const shape of shapes) {
        const name = shape.name.contents;
        if (shape.ensureOnCanvas.contents) {
            const output = constrDict.onCanvas.body(shape, canvas.width, canvas.height).value;
            fns.push({
                ast: {
                    context: {
                        block: { tag: "NamespaceId", contents: "canvas" },
                        subst: { tag: "StySubSubst", contents: {} },
                        locals: im.Map(),
                    },
                    expr: {
...
export const stageConstraints = (inputs, constrFns, objFns, stages) => new Map(stages.map((stage) => [
    stage,
    {
        inputMask: inputs.map((i) => i.stages === "All" || i.stages.has(stage)),
        constrMask: constrFns.map(({ optStages }) => optStages === "All" || optStages.has(stage)),
        objMask: objFns.map(({ optStages }) => optStages === "All" || optStages.has(stage)),
    },
]));
const processPassthrough = ({ symbols }, nameShapeMap) => {
    for (const [key, value] of symbols) {
        ...
        if (shape) {
            if (Object.keys(shape).includes(propName))
                continue;
            if (value.tag === "Val") {
...
export const compileStyleHelper = async (variation, stySource, subEnv, varEnv) => {
    ...
    const canvas = getCanvasDim("width", graph).andThen((w) => getCanvasDim("height", graph).map((h) => makeCanvas(w, h)));
    if (canvas.isErr()) {
    ...
    const inputs = [];
    const metas = [];
    const makeInput = (meta) => {
        const val = meta.init.tag === "Sampled" ? meta.init.sampler(rng) : meta.init.pending;
    ...
    const translation = translate({ makeInput }, canvas.value, optimizationStages.value, graph, assignment.diagnostics.warnings);
    ...
    const groupGraph = makeGroupGraph(getShapesList(translation, [
        ...graph.nodes().filter((p) => typeof graph.node(p) === "string"),
    ]));
    const groupWarnings = checkGroupGraph(groupGraph);
    const { shapeOrdering: layerOrdering, warning: layeringWarning } = computeLayerOrdering([...graph.nodes().filter((p) => typeof graph.node(p) === "string")], [...translation.layering], groupGraph);
    ...
    const renderGraph = buildRenderGraph(findOrderedRoots(groupGraph), groupGraph, nameShapeMap);
    const objFns = [...translation.objectives];
    const constrFns = [
        ...translation.constraints,
        ...onCanvases(canvas.value, shapes),
    ];
    const constraintSets = stageConstraints(metas, constrFns, objFns, optimizationStages.value);
    const computeShapes = await compileCompGraph(inputs, renderGraph);
    const gradient = await genGradient(inputs, objFns.map(({ output }) => output), constrFns.map(({ output }) => output));
...
export const compileStyle = async (variation, stySource, excludeWarnings, subEnv, varEnv) => (await compileStyleHelper(variation, stySource, subEnv, varEnv)).map(({ state }) => ({
    ...state,
    warnings: state.warnings.filter((warning) => !excludeWarnings.includes(warning.tag)),
}));
