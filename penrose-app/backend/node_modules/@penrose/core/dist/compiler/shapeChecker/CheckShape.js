...
import { checkArrow, checkCenter, checkCorner, checkFill, checkNamed, checkPoly, checkProp, checkRect, checkRotate, checkScale, checkString, checkStroke, } from "./CheckShapeHierarchyProps.js";
import { checkClipDataV, checkFloatV, checkPathDataV, checkShapeListV, checkStrV, checkVectorV, } from "./CheckValues.js";
const { err, ok } = Result;
export const checkShape = (shapeType, path, trans) => {
    switch (shapeType) {
        case "Circle":
...
export const checkCircle = (path, trans) => {
...
export const checkEllipse = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
...
export const checkEquation = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const fill = checkFill(path, trans);
...
export const checkGroup = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
...
export const checkImage = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const center = checkCenter(path, trans);
    ...
    if (href.isErr())
        return err(href.error);
    const preserveAspectRatio = checkProp(path, "preserveAspectRatio", trans, checkStrV);
    if (preserveAspectRatio.isErr())
        return err(preserveAspectRatio.error);
    return ok({
        ...named.value,
        ...center.value,
        ...rect.value,
...
export const checkLine = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
    ...
    if (strokeLinecap.isErr())
        return err(strokeLinecap.error);
    const fill = checkFill(path, trans);
    if (fill.isErr())
        return err(fill.error);
    return ok({
...
export const checkPath = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
...
export const checkPolygon = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
...
export const checkPolyline = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
...
export const checkRectangle = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
...
export const checkText = (path, trans) => {
    const named = checkNamed(path, trans);
    if (named.isErr())
        return err(named.error);
    const stroke = checkStroke(path, trans);
    ...
    if (fontFamily.isErr())
        return err(fontFamily.error);
    const fontSizeAdjust = checkProp(path, "fontSizeAdjust", trans, checkStrV);
    if (fontSizeAdjust.isErr())
        return err(fontSizeAdjust.error);
    const fontStretch = checkProp(path, "fontStretch", trans, checkStrV);
    if (fontStretch.isErr())
        return err(fontStretch.error);
    const fontStyle = checkProp(path, "fontStyle", trans, checkStrV);
    if (fontStyle.isErr())
        return err(fontStyle.error);
    const fontVariant = checkProp(path, "fontVariant", trans, checkStrV);
    if (fontVariant.isErr())
        return err(fontVariant.error);
    const fontWeight = checkProp(path, "fontWeight", trans, checkStrV);
    if (fontWeight.isErr())
        return err(fontWeight.error);
    const textAnchor = checkProp(path, "textAnchor", trans, checkStrV);
    if (textAnchor.isErr())
        return err(textAnchor.error);
    const lineHeight = checkProp(path, "lineHeight", trans, checkStrV);
    if (lineHeight.isErr())
        return err(lineHeight.error);
    const alignmentBaseline = checkProp(path, "alignmentBaseline", trans, checkStrV);
    if (alignmentBaseline.isErr())
        return err(alignmentBaseline.error);
    const dominantBaseline = checkProp(path, "dominantBaseline", trans, checkStrV);
    if (dominantBaseline.isErr())
        return err(dominantBaseline.error);
    const ascent = checkProp(path, "ascent", trans, checkFloatV);
    if (ascent.isErr())
        return err(ascent.error);
    const descent = checkProp(path, "descent", trans, checkFloatV);
...
//# sourceMappingURL=CheckShape.js.map