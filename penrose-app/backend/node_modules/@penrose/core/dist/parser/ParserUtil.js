...
export const rangeFrom = (children) => {
...
beginToken, endToken) => {
    ...
    if (!endToken)
        return rangeOf(beginToken);
    if (!beginToken)
        return rangeOf(endToken);
...
// TODO: implement
// }
export const convertTokenId = ([token]) => {
    return {
        ...rangeOf(token),
        value: token.text,
        type: token.type,
...
export const optional = (optionalValue, defaultValue) => (optionalValue ? optionalValue : defaultValue);
...
export const tokensIn = (tokenList) => _.flatten(_.compact(tokenList));
// HACK: locations for dummy AST nodes. Revisit if this pattern becomes widespread.
export const idOf = (value, nodeType) => ({
    nodeType,
    start: { line: 1, col: 1 },
    end: { line: 1, col: 1 },
    tag: "Identifier",
...
export const lastLocation = (parser) => {
...
export const prettyParseError = (e) => {
    const lines = [];
    let inStart = true;
